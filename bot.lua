	...
	if data.ID == "UpdateNewMessage" then
		...
		if msg.content_.ID == "MessageText" then
			...
			if is_naji(msg) then
				...
				elseif text:match("^(وضعیت)$") or match("^(2)$") then
					...
					local txtadd = redis:get("botBOT-IDaddmsgtext") or  "✍ اد شدی 📠📞"
				...
				elseif text:match("^(امار)$") or text:match("^(آمار)$") or match("^(1)$") then
...
📞📠 شماره های ذخیره شده ✍
🆗➡️🚥 <b>]] .. tostring(contacts)..[[</b><code> user </code>
⛓💾 لینک های ذخیره شده : 
...
🆔Ⓜ️ @atash2012]]
				...
				elseif tostring(msg.chat_id_):match("^-") then
					...
					elseif text:match("^(افزودن همه مخاطبین)$") then
						...
						return send(msg.chat_id_, msg.id_, "<i></i>")
					end
				end
			end
		...
		elseif (msg.content_.ID == "MessageContact" and redis:get("botBOT-IDsavecontacts")) then
			...
			if redis:get("botBOT-IDaddmsg") then
				local answer = redis:get("botBOT-IDaddmsgtext") or "✍ اد شدی 📠📞"
